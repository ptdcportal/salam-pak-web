...
export class AccomodationDetailComponent implements OnInit, AfterViewInit {
  ...
  getAccomodationsListing(params: string) {
    this.eventService.getAllPTDCAccomodations(params).then((listing) => {
  ...
  removeFromWishlist() {
    this.eventService
      .deleteAccommodationFromWishlist(this.accommodationId)
      .subscribe((data: any) => {
        this.accommodations.splice(
          this.accommodations.indexOf(this.accommodationId),
          1
        );
      });
  }
  ...
  addToWishlist() {
    ...
    if (this.userId || this.isLoggedIn) {
      this.eventService
        .addAccommodationToWishlist(this.accommodationId)
        .subscribe((data: any) => {
          this.getWishlist();
        });
    } else {

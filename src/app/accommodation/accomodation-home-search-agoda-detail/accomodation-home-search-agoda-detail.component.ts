...
export class AccomodationHomeSearchAgodaDetailComponent implements OnInit {
  ...
  getHotels(location: string, pageSize = 0, skip = 0, filter: any) {
    this.eventService.getAccomodationEvents(location, pageSize, skip, filter).subscribe((data: any) => {
  ...
  getTopHotels(location: string) {
    this.eventService.getAccomodationTopHotels(location, this.pageSize - 5, this.skip).subscribe((data: any) => {
  ...
  removeFromWishlist(eventId: string) {
    this.eventService.deleteAccommodationFromWishlist(eventId).subscribe((data: any) => {
      this.accommodations.splice(this.accommodations.indexOf(eventId), 1);
    });
  }

  removeFromPlan(eventId: string) {
    this.eventService.removeAccommodationFromPlan(eventId).subscribe((data: any) => {
      this.userPlans.splice(this.userPlans.indexOf(eventId), 1);
    });
  }

  addToWishlist(eventId: string) {
    this.eventService.addAccommodationToWishlist(eventId).subscribe((data: any) => {
      this.getWishlist();
    });
  }

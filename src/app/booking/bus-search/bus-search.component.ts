...
export class BusSearchComponent implements OnInit {
  ...
  getBus(data: any, pageSize = 0, skip = 0, filter: any) {
    this.isLoading = true;
    this.eventService
      .getbuss(data, pageSize, skip, filter)
      .then((data: any) => {
  ...
  checkMaleOccupied(seatnumber: any) {
    const exists =
      ...
      this.maleOccoupiedSeats.includes(seatnumber + '');
  ...
  checkFemaleOccupied(seatnumber: any) {
    ...
    const exists =
      ...
      this.femaleOccoupiedSeats.includes(seatnumber + '');
    return exists ? true : false;
  }
  ...
  clearSeatChange() {
    let selectedSeatsDiv = document.querySelectorAll(
      '.row .seat.bus.selected, .row .seat.bus.occupied-female'
    );
  ...
  updateSelectedCount(seatNumber: any) {
    let selectedSeatsDiv = document.querySelectorAll(
      '.row .seat.bus.selected, .row .seat.bus.occupied-female-seat'
    );
  ...
  updateSeatType(seatNumber: any, gender: any) {
    if (gender === 'm') {
      this.selectedMaleSeats = this.addOrRemove(
        this.selectedMaleSeats,
        seatNumber
      );
    } else {
      this.selectedFemaleSeats = this.addOrRemove(
        this.selectedFemaleSeats,
        seatNumber
      );
    }
  ...
  setBusBookingData(data: any, userInfo: any) {
    ...
    this.busBookingData.destination_city_name =
      this.data.destination_city_name + '';

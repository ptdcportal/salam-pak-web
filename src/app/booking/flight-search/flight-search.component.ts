...
export class FlightSearchComponent implements OnInit {
  ...
  data = {  
    legs: [  
       {  
          departureCode: this.getAirportCode(this.route.snapshot.queryParams.from),
          arrivalCode: this.getAirportCode(this.route.snapshot.queryParams.to),
          outboundDate: this.route.snapshot.queryParams.arrival
       }
    ],
    adultsCount:2,
    childrenCount:1,
    ...
    destinationCityNam: airports.filter((i: any) => {
  ...
  from: any = this.route.snapshot.queryParams.from;
  to: any = this.route.snapshot.queryParams.to;
  user: any;
  formFlag = true;
  paymentFlag = false;
  ...
  getAirportCode(text: any)
  {
    const airport = airports.filter((i: any) => {
      return (i.location == text);
    });
    if (airport.length > 0)
    {
      return airport[0].iataCode;
    }
    return false;
  }
  ...
  getFlights(data: any, pageSize = 0, skip = 0, filter: any) {
    ...
    //     this.isLoading = true;
    //     this.allEvents.length = 0;
    //     this.eventService.getFlights(data, pageSize, skip, filter).then((responseData: any) => {
    //       this.isLoading = false;
    //       this.allEvents = responseData && responseData.data.flightItineraries;
    //       // this.refId = responseData && responseData.refId;
    //       // this.pageSize = responseData.meta && responseData.meta.pageSize;
    //       // this.skip = responseData.meta && responseData.meta.skip;
    //       this.total = this.allEvents && this.allEvents ? this.allEvents.length : 0;
    //     });
    //     // this.isLoading = false;
    //     // this.allEvents = responseData && responseData.data.flightItineraries;
    //     // this.refId = responseData && responseData.refId;
    ...
    //     // this.skip = responseData.meta && responseData.meta.skip;
    //     // this.total = this.allEvents && this.allEvents ? this.allEvents.length : 0;
    //     // console.log('Flight data:', this.total, this.allEvents); // Move the console.log here
    // }
    ...
    map((term) =>
      term.length < 2
        ...
        : airports
            .filter(
              (v: any) =>
                // v.airportName.toLowerCase().indexOf(term.toLowerCase()) > -1 ||
                v.location.toLowerCase().indexOf(term.toLowerCase()) > -1
                // v.country.toLowerCase().indexOf(term.toLowerCase()) > -1
            )
            .map((airport: any) => airport.location)
            .slice(0, 10)
    )
  );
  ...
  searchLocation() {
    if (this.bookingFlightForm.valid) {
      let newData = this.bookingFlightForm.value;
      this.data.adultsCount = newData.no_of_adults;
      this.data.childrenCount = newData.no_of_children;
      this.data.infantsCount = newData.no_of_infants;
      this.data.flightTrip = newData.flightTrip;
      this.data.destinationCityName = newData.destinationCityName;
      this.data.departureCityName = newData.departureCityName;
      this.data.dep_date = newData.dep_date;
      this.data.return_date = newData.return_date;
      console.log(this.data.legs[0].departureCode)
      this.data.legs[0].departureCode = this.getAirportCode(newData.departureCityName);
      this.data.legs[0].arrivalCode = this.getAirportCode(newData.destinationCityName);
      this.data.legs[0].outboundDate = newData.dep_date;

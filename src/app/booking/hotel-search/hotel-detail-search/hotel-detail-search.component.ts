...
export class HotelDetailSearchComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.data.hotelId || this.data.hotelRefId) {
      this.eventService.gethotelDetail(this.data).then((data: any) => {
        // console.log(data);
        this.hotelDetail = data && data.hotelDetails;
        this.hotelMedia = data && data.media;
        this.loadScript();
      });
    }
  }

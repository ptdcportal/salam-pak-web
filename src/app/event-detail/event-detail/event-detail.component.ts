...
export class EventDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.credentialsService.currentStatus.subscribe((status: any) => {
      ...
      if (
        status == 'loggedIn' &&
        !this.isBookRoomClicked &&
        !this.isAddToWishlistClicked
      ) {
        this.getWishlist();
  ...
  removeFromWishlist() {
    this.eventService
      .removeEventFromWishlist(this.eventId)
      .subscribe((data: any) => {
  ...
  removeFromPlan() {
    this.eventService
      .removeEventFromPlan(this.eventId)
      .subscribe((data: any) => {
  ...
  addToWishlist() {
    ...
    if (this.userId) {
      this.eventService
        .addEventToWishlist(this.eventId)
        .subscribe((data: any) => {
          this.getWishlist();
        });
    } else {
      const modal = this.ngxSmartModalService.create(
        'authModal',
        AuthModalComponent,
        {
  ...
  navigateOnClick(surrounding: any) {
    ...
    // };
    this.eventService
      .getSurroundings(this.event?.location.coordinates, surrounding.typeKey)
      .then((data: any) => {
  ...
  getLocations() {
    // debugger;
    this.eventService
      .getRelatedEvents(this.event?.cityId)
      .subscribe((locations) => {
  ...
  createBooking() {
    ...
    //  debugger;
    this.eventService
      .createEventBooking(this.eventId, passengersDetail)
      .subscribe(
        (res: any) => {
          // console.log(res);

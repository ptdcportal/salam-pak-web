    ...
    this.eventService.getEvents(params).subscribe((events) => {
      ...
      this.featuredEvents = this.featuredEvents.filter((item: any) => {
        return (
          ...
          (moment(moment(item.date).format('YYYY-MM-DD')).isAfter(moment(new Date()).format('YYYY-MM-DD')) ||
            moment(moment(item.date).format('YYYY-MM-DD')).isSame(moment(new Date()).format('YYYY-MM-DD')))
        );
      });

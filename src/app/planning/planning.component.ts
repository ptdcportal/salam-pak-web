...
export class PlanningComponent implements OnInit, AfterContentChecked {
  ...
  private InitializeDatesForm() {
    ...
    const dates = {
      fromDate: [
        moment(this.plan.startDate).format('YYYY-MM-DD'),
        Validators.required,
      ],
      toDate: [
        moment(this.plan.endDate).format('YYYY-MM-DD'),
        Validators.required,
      ],
    };
  ...
  removeItemFromPlan(planId: any, params: any) {
    this.eventService
      .deleteItemFromPlan(planId, params)
      .subscribe((res: any) => {
  ...
  savePlan() {
    ...
    this.eventService
      .addLocationToSelectedPlan(this.planId, obj)
      .subscribe((res: any) => {

...
export class WishlistComponent implements OnInit {
  ...
  constructor(private readonly eventService: EventService, public ngxSmartModalService: NgxSmartModalService) {
  ...
  removeWishlist(item: any) {
    if (item.type == 'location') {
      ...
      this.eventService.removeTripFromWishlist(item._id).subscribe((data: any) => {
        this.getWishlist();
      });
    } else if (item.type == 'event') {
      this.eventService.removeEventFromWishlist(item._id).subscribe((data: any) => {
        this.getWishlist();
      });
    } else if (item.type == 'accommodation') {
      this.eventService.deleteAccommodationFromWishlist(item._id).subscribe((data: any) => {
        this.getWishlist();
      });
    }
  }
